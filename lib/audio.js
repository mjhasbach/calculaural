...
let audio = {
        ...
        notes: {
            ...
            generate: function(instrumentCursor) {
                let notes = [],
                    ...
                    {tone, duration, notesPerMeasure, tonic, chordType, arpeggiateChord, arpeggioDirection, arpeggioPeakValley} = opt,
                    polyphony = Math.ceil(notesPerMeasure / parseInt(duration)),
                    voices = polyphony,
                ...
                for (let digit of digits) {
                    ...
                    if (chordType !== 'none' && scale) {
                        ...
                        notes.push([time, note.fq(), duration]);
                    }
    ...
    playback: {
        ...
        seek(direction) {
            let i = 1,
                wasPlaying = Tone.Transport.state !== 'stopped';
            ...
            (function seek() {
                ...
                if (audio.playback.seeking) {
                    ...
                    for (let instrument of audio.context.cursors.instruments.get()) {
                        instrument.pianoRoll.playback.seek(audio.playback._position);
                    }
